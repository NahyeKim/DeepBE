...
final_model =  tf.keras.models.load_model('DeepBE_17m_model',compile=False)
...
for x in range(short_length):
    ...
    for i in range(1,2**x+1):
        for k in range(1,2**(7-x)+1):
            b.append(2**(8-x)*i-k-1)
    alist.append(b)
...
for i in range(len(hyperparameter_prediction)):
    ...
    for j in range(len(non)):
        ...
        for k in iter:
            on.append(sum(k))
    on = [x - 1 for x in on]
    off = list(set(list(range(255)))-set(on))
    for m in off:
...
seq = pd.DataFrame(np.repeat(dataset_all['target + PAM'].reset_index(drop=True).values,255,axis=0))
pam = pd.DataFrame(np.repeat(dataset_all['feature'].reset_index(drop=True).values,255,axis=0))
...
for i in range(int(len(hyperparameter_prediction)/255)):
    for k in range(len(alist)):
        for j in alist[k]:
            hyperparameter_prediction.iloc[i+j,1] = hyperparameter_prediction.iloc[i+j,1][:k+6] + "g" + hyperparameter_prediction.iloc[i+j,1][k+1+6:]
